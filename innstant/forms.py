...
from .models import Complaint, Rules, Comment, TenantInfo, Agreement, Test, Room, RoomType, ComplaintType, Payment, \
...
class PaymentForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fields['tenant'].queryset = self.fields['tenant'].queryset.filter(Q(room_user__isnull=True) | Q(room_user=None), is_active=True, is_superuser=False)
        self.fields['tenant'].label_from_instance = lambda obj: f"{obj.first_name} {obj.last_name}"
...
class TestForm(forms.ModelForm):
    class Meta:
        model = Test
        fields = ('test_1', 'test_Description')

        widgets = {
            'test_1': forms.TextInput(attrs={'class': 'form-control form-control-sm', 'placeholder': ' '}),
            'test_Description': forms.Textarea(attrs={'class': 'form-control form-control-sm', 'placeholder': ' '}),
        }