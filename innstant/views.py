...
from .forms import ComplaintForm, RuleForm, AddUserForm, CommentForm, TestForm, TenantInfoForm, AgreementForm,\
...
from . models import Complaint, Rules,  Room, User, TenantInfo, Agreement, Test,RoomType, ComplaintType, Payment, \
...
def send_email(request, id):
    ...
    payment = Payment.objects.get(room=room)
...
def edit_footer_admin(request):
    try:
        footer = Footer.objects.get(pk=4)
...
def admin_QRCODE(request):
    try:
        qrcodes = QRCODEPayment.objects.all()
...
def agreement_pdf_admina(request, id):
...
def export_pdf(request):
    ...
    response['Content-Disposition'] = 'inline; attachment; filename=Test' + \
...
def save_agreement_admin(request):
    # Create a file-like buffer to receive PDF data.
    buffer = io.BytesIO()
